<?php
  ...
  include_once "func.php";
    ...
    if($_SERVER['REQUEST_METHOD']=='POST' && $_POST['action']=='delete'){
   	    ...
   	    $sql="DELETE  FROM `area` WHERE area_code='$code'";
  ...
  if($_SERVER['REQUEST_METHOD']=='POST' && $_POST['action']=='save'){
   	    ...
   	    //check custcode availibilty
   	     if( is_area_code_exist($code)) {
   	     	 echo "This Area Code had been taken.";
   	     	 exit;
   	     } 
   	    ...
   	    $sql="INSERT INTO `area`(area_code,area_desc,active) VALUES ('$code','$desc','$active')";
   	    $result=mysql_query($sql);
   	    if(!$result) die("MySQL Error: ".mysql_error());
   	    exit;
    ...
    if($_SERVER['REQUEST_METHOD']=='POST' && $_POST['action']=='update'){
  	    ...
  	    if($active) $active='Y';
   	    ...
   	    $sql="UPDATE `area` SET area_desc='$desc',active='$active' WHERE area_code='$code'";
   	    $result=mysql_query($sql);
   	    if(!$result) die("MySQL Error: ".mysql_error());
   	    exit;
    ...
    if($_SERVER['REQUEST_METHOD']=='POST' && ($_POST['action']=='view' || $_POST['action']=='edit')){
    	$code=$_POST['code'];
    	$sql="SELECT area_code,area_desc,active from `area` WHERE area_code='$code'";
    	$result=mysql_query($sql);
    	if(!$result) die('MySql Error: '.mysql_error());
...
<div class='form'>
<button style='float:right;' onclick="open_menu('area_form',{action:'edit',code:'<?php echo $data[0]; ?>'});" type='submit'>Edit</button><button onclick="open_menu('area_browse');" ><img src='../images/back.gif' width="12px" height="12px" align="absmiddle" />&nbsp;Back</button>
<div class='form body'>
      <table>
      <tr><td>Area Code:</td><td><label class="textbox"><?php echo $data['0']; ?></label></td></tr>
...
<button onclick="open_menu('area_browse');return false;">Cancel</button>
 ...
 <div style='float:right'>
    ...
    <table>
      ...
      <tr><td>Description:</td><td><input maxlength='150' id='desc' name='desc' value="<?php echo $data[1]; ?>" /></td></tr> 
       <tr><td></td><td> <button style="width:250px;" class='form deletebutton'>Delete Area</button></td></tr>
    </table>
...
$(function(){
	$('#frm').validate({
		...
		messages:{
	    desc:"Please enter the description for this Area Code",
	   },
  ...
  $('#deletedialog').aodialog({
  	okay: function(){
		...
		$.post('../callback/area_form.php',{code:'<?php echo $data[0]; ?>',action:'delete'},function(e){
			   $('#deletedialog').aodialog('close');
	   	       
  	   		   open_menu('area_browse');	
	...
	$('#savedialog').aodialog({
  	okay: function(){
	   ...
	   $.post('../callback/area_form.php',$('#frm').serialize(),function(e){
...
<div class='form body'>
 ...
 <div style='float:right'>
    ...
    <table>
      <tr><td>Area Code:</td><td><input maxlength='10' id='code' name='code'  /></td></tr>
...
$(function(){
	...
	$('#frm').validate({
		...
		messages:{
		code: "Please enter an unique area Code.",
	    desc:"Please enter the description for this area Code",
	   },
	   submitHandler: function(form) {
	   	     load_busy('.contentbox .c');
      	    ...
      	    $.post('../callback/area_form.php',$(form).serialize(),function(e){
      	     	if($.trim(e)!=""){
