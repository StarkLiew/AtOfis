<?php
  ...
  include_once "func.php";
    ...
    if($_SERVER['REQUEST_METHOD']=='POST' && $_POST['action']=='delete'){
   	    ...
   	    $sql="DELETE  FROM `brand` WHERE brand_code='$code'";
  ...
  if($_SERVER['REQUEST_METHOD']=='POST' && $_POST['action']=='save'){
   	    ...
   	    //check custcode availibilty
   	     if( is_brand_code_exist($code)) {
   	     	 echo "This brand Code had been taken.";
   	     	 exit;
   	     } 
   	    ...
   	    $sql="INSERT INTO `brand`(brand_code,brand_desc,active) VALUES ('$code','$desc','$active')";
   	    $result=mysql_query($sql);
   	    if(!$result) die("MySQL Error: ".mysql_error());
   	    exit;
    ...
    if($_SERVER['REQUEST_METHOD']=='POST' && $_POST['action']=='update'){
  	    ...
  	    if($active) $active='Y';
   	    ...
   	    $sql="UPDATE `brand` SET brand_desc='$desc',active='$active' WHERE brand_code='$code'";
   	    $result=mysql_query($sql);
   	    if(!$result) die("MySQL Error: ".mysql_error());
   	    exit;
    ...
    if($_SERVER['REQUEST_METHOD']=='POST' && ($_POST['action']=='view' || $_POST['action']=='edit')){
    	$code=$_POST['code'];
    	$sql="SELECT brand_code,brand_desc,active from `brand` WHERE brand_code='$code'";
    	$result=mysql_query($sql);
    	if(!$result) die('MySql Error: '.mysql_error());
...
<div class='form'>
<button style='float:right;' onclick="open_menu('brand_form',{action:'edit',code:'<?php echo $data[0]; ?>'});" type='submit'>Edit</button><button onclick="open_menu('brand_browse');" ><img src='../images/back.gif' width="12px" height="12px" align="absmiddle" />&nbsp;Back</button>
<div class='form body'>
      <table>
      <tr><td>brand Code:</td><td><label class="textbox"><?php echo $data['0']; ?></label></td></tr>
...
<button onclick="open_menu('brand_browse');return false;">Cancel</button>
 ...
 <div style='float:right'>
    ...
    <table>
      ...
      <tr><td>Description:</td><td><input maxlength='150' id='desc' name='desc' value="<?php echo $data[1]; ?>" /></td></tr> 
       <tr><td></td><td> <button style="width:250px;" class='form deletebutton'>Delete Brand</button></td></tr>
    </table>
...
$(function(){
	$('#frm').validate({
		...
		messages:{
	    desc:"Please enter the description for this Brand Code",
	   },
  ...
  $('#deletedialog').aodialog({
  	okay: function(){
		...
		$.post('../callback/brand_form.php',{code:'<?php echo $data[0]; ?>',action:'delete'},function(e){
			   $('#deletedialog').aodialog('close');
	   	       
  	   		   open_menu('brand_browse');	
	...
	$('#savedialog').aodialog({
  	okay: function(){
	   ...
	   $.post('../callback/brand_form.php',$('#frm').serialize(),function(e){
...
<div class='form body'>
 ...
 <div style='float:right'>
    ...
    <table>
      <tr><td>brand Code:</td><td><input maxlength='10' id='code' name='code'  /></td></tr>
...
$(function(){
	...
	$('#frm').validate({
		...
		messages:{
		code: "Please enter an unique Brand Code.",
	    desc:"Please enter the description for this Brand Code",
	   },
	   submitHandler: function(form) {
	   	     load_busy('.contentbox .c');
      	    ...
      	    $.post('../callback/brand_form.php',$(form).serialize(),function(e){
      	     	if($.trim(e)!=""){
